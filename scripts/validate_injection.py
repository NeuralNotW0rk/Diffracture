...
def run_validation_test(model: nn.Module, grating_ckpt: str):
    """
    Main execution flow to validate Grating injection into a target model.
    """
    print(f"--- Starting Validation Test ---")
    
    print(f"Loading Grating from {grating_ckpt}...")
    grating = Grating.load(grating_ckpt)
    ...
    print("Activating...")
    actant.activate(grating, injection_strategy="graft")
...
if __name__ == "__main__":
    ...
    grating_ckpt = model_dir / "diffracture" / "loras" / f"{lora_name}.safetensors"
    ...
    run_validation_test(model, str(grating_ckpt))